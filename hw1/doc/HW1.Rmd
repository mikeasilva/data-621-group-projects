...
We will fill in the values with KNN


```{r}
impute_me <- is.na(training$TEAM_BATTING_HBP)
training[impute_me,"TEAM_BATTING_HBP"] <- knn[impute_me,"TEAM_BATTING_HBP"] 
```
...
We can use the KNN imputation to help with the NA values.  We can turn our attention now to the extreme values.
...
We'll make a high-yet-reasonable assumption of a mean 11 innings per game, and call anything more than 5,346 strikeouts an invalid data point by setting them to NA so they will be imputed prior to modeling.

```{r}
training[which(training$TEAM_PITCHING_SO > 5346),"TEAM_PITCHING_SO"] <- NA
...
We will select a subset of variables.

```{r}
fit_final <- lm(TARGET_WINS ~ TEAM_BATTING_H + TEAM_BATTING_BB + TEAM_BATTING_HBP + TEAM_FIELDING_E, data = moneyball_train)
summary(fit_final)
```

## Model Selection

In order to select which model is the "best" we will test it against a test set.  We will examine the difference between the predicted and actual.
...
summary(moneyball_test$error)
```

### Select Model

```{r}
moneyball_test$y_hat <- predict(fit_final, moneyball_test)
moneyball_test <- moneyball_test %>%
  mutate(error = TARGET_WINS - y_hat) %>%
  mutate(squared_error = error ^ 2)
