...
density_diff <- function(data,var,target,new_column){
#standardize variable between 0 and 1
var <- (var - min(var))/(max(var) - min(var))
## Calculate density estimates
g1 <- ggplot(data, aes(x=var, group=target, colour=target)) +
  geom_density(data = data) + xlim(min(var), max(var))
gg1 <- ggplot_build(g1)
...
new_data <- merge(means,data, by.x ="std_var", by.y ="key")
...
mylist <- list(g1,g3,new_data)
names(mylist)<- c("dist","dist_diff","new_data_training")
return(mylist)

}

new_data2 <- density_diff(df,df$nox,df$target,"nox_density")$new_data_training

### do it now with a way to include evaluation data
