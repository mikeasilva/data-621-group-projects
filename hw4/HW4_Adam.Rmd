...
baseline_model <- lm(log(TARGET_AMT+1) ~ BLUEBOOK -1, train)
summary(baseline_model)
```
...
```{r}
car_model <- lm(log(TARGET_AMT+1) ~ CAR_AGE + CAR_TYPE + CAR_USE, data = train)
summary(car_model)
```
...
```{r}
personal_model <- lm(log(TARGET_AMT+1) ~ MSTATUS + INCOME + KIDSDRIV +
                       log(HOME_VAL+1) + AGE + REVOKED + MVR_PTS +
                       HAS_KIDS, data= train)
...
```{r}
add_model <- lm(1/sqrt(TARGET_AMT+1) ~ BLUEBOOK + CAR_AGE,
                data = train)
