...
hw4_categorical_plot <- function(df, var, strip_dollar_signs = FALSE){
  ...
  plot_1 <-  temp %>%
    ggplot(.) +
    geom_bar(aes(X, PERCENT, fill = TARGET_FLAG), stat="identity") +
...
There are `r nrow(train[TARGET_FLAG == 1,])` out of `r nrow(train)` records in the training data set that have been in an accident.  We want to correct the imballance in the dataset by over sampling this group so our model will do a better job identifying this minority group.
...
over_sample_train <- data.frame(INDEX = sample(minority_index, diff, T)) %>%
  merge(train, .) %>%
  bind_rows(train)
```
