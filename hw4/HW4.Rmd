...
Here we add meaningful variables to the model until we have a robust model with a decent AUC value.

```{r, null_prefix = TRUE, comment=NA}
model <- glm(TARGET_FLAG ~ PRIOR_ACCIDENT + KID_DRIVERS + MSTATUS + LOG_INCOME +
             SEX + CAR_USE + CAR_AGE + COLLEGE_EDUCATED + REVOKED + URBAN_DRIVER +
               TIF + TRAVTIME + AGE,
             family = binomial(link = "logit"), over_sample_train)
summary(model)
