...
```{r, null_prefix = TRUE, comment=NA}
outlier_model <- glm(TARGET_AMT_OUTLIER ~ BLUEBOOK + MVR_PTS + JOB, family = binomial(link = "logit"), over_sample_train_2)
summary(outlier_model)
outlier_model_results <- evaluate_model(outlier_model, test, "TARGET_AMT_OUTLIER", 0.5)
```
...
```{r}
temp <- over_sample_train_2 %>%
  select(-TARGET_FLAG, - TARGET_AMT)
fft <- FFTrees(TARGET_AMT_OUTLIER~., temp)
fft
```
