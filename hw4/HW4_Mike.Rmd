...
```{r}
over_sample <- function(df, predictors){
  vars <- c("INDEX", "TARGET_FLAG", predictors)
  df <- df[vars] %>%
    na.omit()
  set.seed(42)
  minority <- nrow(df[df$TARGET_FLAG == 1,])
  majority <- nrow(df[df$TARGET_FLAG == 0,])
  diff <- majority - minority
  minority_index <- df[df$TARGET_FLAG == 1,]$INDEX
  over_sample_df <- data.frame(INDEX = sample(minority_index, diff, TRUE)) %>%
    merge(df, .) %>%
    bind_rows(df)
  return(over_sample_df)
}
...
temp <- over_sample(train, c("PRIOR_ACCIDENT"))
baseline_model <- glm(TARGET_FLAG ~ PRIOR_ACCIDENT, family = binomial(link = "logit"), temp)
...
```{r, null_prefix = TRUE, comment=NA}
risk_taker_model <- glm(TARGET_FLAG ~ RED_SPORTS_CAR + YOUNG_MALE, family = binomial(link = "logit"), over_sample(train, c("RED_SPORTS_CAR", "YOUNG_MALE")))
...
traditional_model <- glm(TARGET_FLAG ~ YOUNG + MSTATUS + PRIOR_ACCIDENT + SEX +
             ...
             family = binomial(link = "logit"), over_sample(train, c("YOUNG", "MSTATUS", "PRIOR_ACCIDENT", "SEX", "REVOKED", "MVR_PTS", "TRAVTIME", "CAR_USE")))
...
This model has statistically significant predictors.  Applying this model to the test data set indicates this simple model has a `r  round(unname(model_results$overall['Accuracy'])*100,1)`% accuracy rate.  It correcly identified `r round(unname(model_results$byClass['Sensitivity'])*100,1)`% of the people with accidents and `r  round(unname(model_results$byClass['Specificity'])*100,1)`% of those without.  This model out preforms the baseline model.
...
model <- glm(TARGET_FLAG ~ PRIOR_ACCIDENT + KID_DRIVERS + MSTATUS + INCOME +
             ...
             family = binomial(link = "logit"), over_sample(train, c("PRIOR_ACCIDENT", "KID_DRIVERS", "MSTATUS", "INCOME", "SEX", "CAR_USE", "COLLEGE_EDUCATED", "REVOKED", "URBAN_DRIVER")))
summary(model)
...
To select build a model of how much the claim amount will be we need to subset the training data to only include those that have accidents.  There will be `r train %>% filter(TARGET_FLAG == 1) %>% nrow()` data points in this set.
...
It looks like `BLUEBOOK` is the only numeric variable that correlates with the claim amount.  We will use it as the baseline and see if we can improve this models.
...
```{r}
evaluation$TARGET_FLAG <- ifelse(predict.glm(model, evaluation, "response") >= 0.5, 1, 0)
# We assume that everyone with a TARGET_FLAG = 0 has a TARGET_AMT as zero.  
# Urban drivers will use one model, rural another
evaluation$urban_yhat <- predict(urban_fit, evaluation)
evaluation$rural_yhat <- predict(rural_fit, evaluation)
evaluation <- evaluation %>%
  mutate(TARGET_AMT = ifelse(URBAN_DRIVER == 1, urban_yhat, rural_yhat)) %>%
