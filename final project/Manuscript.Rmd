...
temp <- rbind(data.frame("Model" = c("2"), "Specificity" = model2_robust_results$specificity))%>%
  mutate(Model = as.factor(Model))

ggplot(temp, aes(x = Specificity, fill = Model, color = Model)) +
  geom_density(alpha = 0.5) +
  ylab("Density") +
  scale_fill_brewer(palette = "Set1") +
  theme(legend.position = "bottom") +
  #geom_vline(xintercept = mean(model1_robust_results$specificity), linetype="dashed", color = "#e41a1c") +
  geom_vline(xintercept = mean(model2_robust_results$specificity), linetype="dashed", color = "#377eb8")
```
